...
# Recent socket events for admin debugging
...
# Minimal config and helper stubs to avoid circular imports.
from dataclasses import dataclass, field
from typing import List, Dict, Any, Optional

console = Console()

# Rate limit defaults for simple bot protection
RATE_LIMIT_WINDOW = int(os.getenv('RATE_LIMIT_WINDOW', '60'))  # seconds
RATE_LIMIT_MAX = int(os.getenv('RATE_LIMIT_MAX', '5'))


def generate_unique_id(prefix: str = 'JD') -> str:
    """Generate a short unique tracking-like ID (prefix + 10 digits)."""
    import random
...
def search_shipments(query: str, page: int = 1, per_page: int = 10):
    """Lightweight fallback search that returns empty results when DB access is not available.
    This keeps the bot operational in environments without a configured database.
    """
    try:
        # Best-effort: try to use SQLAlchemy Shipment model if present
        if 'db' in globals() and 'Shipment' in globals():
            with app.app_context():
                q = f"%{query}%"
                results = Shipment.query.filter(
                    (Shipment.tracking_number.like(q)) | (Shipment.delivery_location.like(q)) | (Shipment.origin_location.like(q))
...
def get_recent_logs(limit: int = 10):
    """Return last `limit` lines from a log file if present (fallback)."""
    try:
...
class BotConfig:
    ...
    valid_statuses: List[str] = field(default_factory=lambda: [
        "Pending", "On_Hold", "In_Transit", "Out_for_Delivery", "Delivered", "Returned", "Delayed"
    ])
...
# Redis client placeholder and helpers
redis_client = None
...
def get_redis_client():
    ...
    try:
        ...
        if not url:
            return None
        # Create a connection pool with a configurable max_connections to avoid hitting Redis client limits
...
# Safe Redis wrappers to avoid throwing in background threads
...
def r_lpop(key):
    ...
    try:
        ...
        # disable redis client on fatal errors to let the app use in-memory fallback
        try:
            redis_client = None
        except Exception:
            pass
...
# Simulation run guard to avoid duplicate simulator processes per tracking number
_running_simulations = set()
_running_simulations_lock = threading.Lock()

def spawn_simulation(tn):
    """Spawn a simulate_tracking runner only if not already running for tn."""
    if not tn:
        return False
    ...
    def _runner():
        try:
            try:
                ...
                # simulate_tracking may raise; ensure it's logged by caller
                raise
        finally:
            with _running_simulations_lock:
...
def cache_route_templates():
    ...
    try:
        _route_templates_cache = BotConfig().route_templates or {}
    except Exception:
        _route_templates_cache = {}

def get_cached_route_templates() -> Dict[str, Any]:
    return _route_templates_cache or BotConfig().route_templates
...
def get_shipment_list(*args, **kwargs):
    # Lazy DB-backed implementation to avoid circular imports at module load time
    try:
        from app import app, db, Shipment
        page = kwargs.get('page', 1) if kwargs else args[0] if args else 1
...
def get_shipment_details(*args, **kwargs):
    try:
        tn = args[0] if args else kwargs.get('tracking_number')
        if not tn:
            return None
        from app import app, db, Shipment
        with app.app_context():
...
def save_shipment(*args, **kwargs):
    try:
        tracking_number = args[0] if args else kwargs.get('tracking_number')
        status = args[1] if len(args) > 1 else kwargs.get('status')
        checkpoints = args[2] if len(args) > 2 else kwargs.get('checkpoints', '')
        delivery_location = args[3] if len(args) > 3 else kwargs.get('delivery_location')
        recipient_email = kwargs.get('recipient_email')
        origin_location = kwargs.get('origin_location')
        webhook_url = kwargs.get('webhook_url')
        carrier = kwargs.get('carrier', 'DHL')
        if not tracking_number or not delivery_location or not status:
            return False
        from app import app, db, Shipment
...
def send_dynamic_menu(chat_id, page: int = 1, message_id: int = None):
    """Send or edit a simple paginated shipment menu to `chat_id` using the configured bot."""
    try:
        bot = get_bot()
        if not bot:
            return None
...
def show_shipment_menu(call_or_chat, page: int = 1, mode: str = 'view', title: str = 'Select shipment'):
    try:
        # call_or_chat can be a callback call or a chat id
...
def verify_recaptcha(token):
    # Disable reCAPTCHA in development or when Flask debug is enabled
    if app.debug or app.config.get('FLASK_ENV') == 'development':
        return True

    # If default placeholder secret is present, treat as disabled
    if 'your-secret-key' in app.config.get('RECAPTCHA_SECRET_KEY', ''):
        return True
...
def geocode_locations(checkpoints):
    ...
    for cp in checkpoints:
        ...
        try:
            ...
            if coord:
                geocode_cache[cp] = coord
                r_set(cache_key, json.dumps(coord), ex=86400)
                coords.append(coord)
        except Exception:
            pass
    return coords
    ...
    while True:
        ...
        try:
            ...
            if typ == "email":
                ...
                requests.post(d["webhook_url"], json={**d, "tracking_number": data.get("tracking_number")}, timeout=10)
        except Exception as e:
...
def cleanup_websocket_clients():
    while True:
        ...
        for key in r_scan_iter("clients:*"):
            tn = key.split(":", 1)[1]
            for sid in r_smembers(key):
                try:
...
def track_metrics(tn, event_type, data):
    ...
    # use safe list append for metrics
    key = f"metrics:{tn}"
    r_lpush(key, json.dumps({
                ...
                if new_status != current_status or random.random() < 0.12:
                    ...
                    if stage == "pickup":
                        ...
                        checkpoint = DHLRealisticSimulator.generate_realistic_checkpoint(
                            city, new_status, tn, destination=destination
                        )
                    elif stage == "delivery":
                        delivery_events = [
...
def basic_simulate_tracking(tn):
    ...
    from app import rget
    transport_mode = (rget("transport_mode", tn, default_mode) or default_mode)
        ...
        try:
            ...
            if tracking_number:
                try:
                    ...
                    # store email history using safe redis wrappers
                    r_lpush(history_key, json.dumps(entry))
                    r_ltrim(history_key, 0, 99)
...
def broadcast_update(tn):
    ...
    if not shipment:
        return
    speed = float(r_hget("sim_speed_multipliers", tn) or "1.0")
    paused = (r_hget("paused_simulations", tn) == "true")
    try:
...
def admin_metrics():
    metrics = {}
    if redis_client:
        active_keys = r_keys("clients:*")
        metrics['active_simulations'] = len(active_keys)
        speeds = r_hgetall("sim_speed_multipliers") or {}
        if speeds:
            try:
                metrics['avg_speed'] = round(sum(float(v) for v in speeds.values()) / len(speeds), 2)
            except Exception:
                metrics['avg_speed'] = 0.0
        metrics['paused_simulations'] = r_hlen("paused_simulations") if r_exists("paused_simulations") else 0
    ...
    try:
        ...
        for s in shipments_query.items:
            try:
                ...
                try:
                    paused = (r_hget("paused_simulations", s.tracking_number) == "true")
                    speed = float(r_hget("sim_speed_multipliers", s.tracking_number) or "1.0")
                    mode = r_hget("transport_mode", s.tracking_number) or "ground"
                    progress = float(r_hget("progress", s.tracking_number) or "0")
                    stage = r_hget("stage", s.tracking_number) or "pickup"
                    sl = r_hget("service_level", s.tracking_number)
                    if sl:
                        service_level = sl
                    dw = r_hget("delivery_window", s.tracking_number)
                    if dw:
                        delivery_window = dw
                    pod = r_hget("proof_of_delivery", s.tracking_number)
                    if pod:
        ...
        return render_template('admin_dashboard.html',
                               total=total,
                               queue_len=r_llen("notifications"),
                               active_clients=len(r_keys("clients:*")),
...
def api_shipment_detail(tn):
    ...
    delivery_window = rget("delivery_window", tn)
    proof_of_delivery = rget("proof_of_delivery", tn)
    temperature = rget("temperature", tn)
...
def api_create_shipment():
    try:
        ...
        now = datetime.now()
        checkpoints = f"{now.strftime('%Y-%m-%d %H:%M')} - {origin} - Shipment information received"
        ...
        # Verify it was saved
        saved = Shipment.query.filter_by(tracking_number=tracking_number).first()
        if not saved:
            flask_logger.error(f"Failed to save shipment {tracking_number} to database")
            return jsonify({'error': 'Failed to save shipment to database'}), 500
        
        flask_logger.info(f"Shipment {tracking_number} saved successfully")
        ...
        if redis_client:
            try:
                ...
                r_hset('service_level', tracking_number, service_level)
                r_hset('transport_mode', tracking_number, mode)
                r_hset('delivery_attempts', tracking_number, '0')
                r_hset('max_attempts', tracking_number, str(max_attempts))
                r_hset('progress', tracking_number, '0')
                r_hset('stage', tracking_number, 'pickup')
                r_hset('delivery_window', tracking_number, delivery_window)
                r_hset('proof_of_delivery', tracking_number, 'Pending')
            except Exception as redis_err:
...
# Keep remaining admin API endpoints (send_email, email_history, pause, speed, mode, bulk_create, active_shipments, carrier)
...
def on_connect():
    ...
    flask_logger.info(
        "SocketIO connect (utils): sid=%s addr=%s transport=%s headers=%s",
        sid,
        request.remote_addr,
        transport,
        {k: headers.get(k) for k in ['User-Agent', 'Origin', 'Referer'] if headers.get(k)}
    )
...
def on_disconnect():
    sid = getattr(request, 'sid', None)
    flask_logger.info("SocketIO disconnect (utils): sid=%s addr=%s", sid, request.remote_addr)
...
def on_request(data):
    ...
    speed = float(r_hget("sim_speed_multipliers", tn) or "1.0")
    paused = (r_hget("paused_simulations", tn) == "true")
    mode = r_hget("transport_mode", tn) or ("air" if estimate_distance(shipment.origin_location or "Lagos, NG", shipment.delivery_location) > 1000 else "ground")
    distance_km = estimate_distance(shipment.origin_location or "Lagos, NG", shipment.delivery_location)
    service_level = DHLRealisticSimulator.get_service_level(
        distance_km, DHLRealisticSimulator.is_business_hours(datetime.now())
    )
