...
# Import dependencies from app.py
from app import db, Shipment, sanitize_tracking_number, validate_email, validate_location, validate_webhook_url, send_email_notification, console, paused_simulations, sim_speed_multipliers
from config import WEBSOCKET_SERVER, VALID_STATUSES, ALLOWED_ADMINS

# Initialize logging and Telegram bot
bot_logger = logging.getLogger('telegram_bot')
bot = telebot.TeleBot('YOUR_TELEGRAM_BOT_TOKEN')  # Replace with actual bot token
console = Console()

# Cache for route templates to avoid hardcoding routes repeatedly
route_templates = {}

# Cache for shipment details to reduce database queries
shipment_cache = {}
...
def save_shipment(tracking_number, status, checkpoints, delivery_location, recipient_email='', origin_location=None, webhook_url=None, email_notifications=True):
    ...
    try:
        ...
        db.session.commit()
        # Update cache
        shipment_cache[sanitized_tn] = shipment.to_dict()
        shipment_cache[sanitized_tn]['paused'] = paused_simulations.get(sanitized_tn, False)
        shipment_cache[sanitized_tn]['speed_multiplier'] = sim_speed_multipliers.get(sanitized_tn, 1.0)
...
def stop_simulation(message):
    ...
    try:
        ...
        if shipment['status'] in ['Delivered', 'Returned']:
            ...
            return
        if tracking_number in paused_simulations and paused_simulations[tracking_number]:
        ...
        shipment_cache[tracking_number]['paused'] = True
...
def continue_simulation(message):
    ...
    if not tracking_number:
        ...
        return
    try:
        if tracking_number not in paused_simulations or not paused_simulations[tracking_number]:
        ...
        shipment_cache[tracking_number]['paused'] = False
...
def set_simulation_speed(message):
    ...
    try:
        ...
        shipment_cache[tracking_number]['speed_multiplier'] = speed
...
def handle_reply_input(message):
    ...
    if "Enter shipment details" in original_message:
        ...
        action = "update"
        tracking_number = sanitize_tracking_number(original_message.split("Enter updates for ")[1].split(":")[0])
    elif "Enter simulation speed for" in original_message:
        action = "setspeed"
        tracking_number = sanitize_tracking_number(original_message.split("Enter simulation speed for ")[1].split(" ")[0])
...
def handle_add_input(message):
    """Handle input for adding a new shipment."""
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "Access denied.")
    ...
    if message.text.lower() == "cancel":
        ...
        return
    try:
        # Expected format: [tracking_number] <status> "<checkpoints>" <delivery_location> [recipient_email] [origin_location] [webhook_url]
        parts = re.match(r'(\S+)\s+(\S+)\s+"([^"]*)"\s+(\S+(?:,\s*\S+)?)(?:\s+(\S+@[\w.-]+\.\w+))?(?:\s+(\S+(?:,\s*\S+)?))?(?:\s+(\S+))?', message.text.strip())
        if not parts:
            bot.reply_to(message, "Invalid format. Use: `[tracking_number] <status> \"<checkpoints>\" <delivery_location> [recipient_email] [origin_location] [webhook_url]`\nExample: `TRK123 In_Transit \"2025-10-14 13:00 - Lagos, NG - Processed\" \"Lagos, NG\" user@example.com \"Abuja, NG\" https://webhook.site`", parse_mode='Markdown')
            bot_logger.warning("Invalid add shipment format", extra={'tracking_number': ''})
            return
        tracking_number, status, checkpoints, delivery_location, recipient_email, origin_location, webhook_url = parts.groups(default='')
        checkpoints = checkpoints or ''
        recipient_email = recipient_email or ''
        origin_location = origin_location or None
        webhook_url = webhook_url or None
        save_shipment(
...
def callback_query(call):
    ...
    page = int(call.data.split('_')[-1]) if call.data.startswith(('view_menu_', 'update_menu_', 'delete_menu_', 'batch_delete_menu_', 'broadcast_menu_', 'toggle_email_menu_', 'pause_menu_', 'resume_menu_', 'setspeed_menu_', 'getspeed_menu_', 'list_', 'menu_page_')) else 1
    ...
    actions = {
        ...
        'delete': lambda tn: delete_shipment(call, tn, page),
        ...
        'batch_select': lambda tn: toggle_batch_selection(call, tn),
        ...
        'getspeed': lambda tn: show_simulation_speed(call, tn),
    ...
    try:
        ...
        tracking_number = sanitize_tracking_number(call.data.split('_', 1)[1]) if '_' in call.data and call.data.startswith(('view_', 'update_', 'delete_', 'batch_select_', 'broadcast_', 'toggle_email_', 'pause_', 'resume_', 'setspeed_', 'getspeed_')) else None
        handler = actions.get(action, lambda: bot.answer_callback_query(call.id, "Unknown action.", show_alert=True))
        handler() if tracking_number is None else handler(tracking_number)
    except Exception as e:
        bot.answer_callback_query(call.id, f"Error: {e}", show_alert=True)
        bot_logger.error(f"Error in callback query: {e}", extra={'tracking_number': tracking_number or ''})
...
def show_shipment_details(call, tracking_number):
    ...
    if details:
        response = (
            ...
            f"*Origin Location*: `{details['origin_location'] or 'None'}`\n"
...
def delete_shipment(call, tracking_number, page):
    ...
    try:
        ...
        if shipment:
            ...
            if tracking_number in shipment_cache:
                del shipment_cache[tracking_number]
    ...
    try:
        ...
        for tn in batch_list:
            ...
            if shipment:
                ...
                if tn in shipment_cache:
                    del shipment_cache[tn]
                deleted_count += 1
                try:
...
def toggle_email_notifications(call, tracking_number, page):
    ...
    try:
        ...
        if not shipment:
            ...
            bot_logger.warning(f"Shipment not found: {tracking_number}", extra={'tracking_number': tracking_number})
            return
        shipment_db = Shipment.query.filter_by(tracking_number=tracking_number).first()
        shipment_db.email_notifications = not shipment_db.email_notifications
        db.session.commit()
        shipment_cache[tracking_number]['email_notifications'] = shipment_db.email_notifications
        status = "enabled" if shipment_db.email_notifications else "disabled"
...
def pause_simulation_callback(call, tracking_number, page):
    ...
    try:
        ...
        bot_logger.error(f"Database error pausing simulation: {e}", extra={'tracking_number': tracking_number})
        console.print(Panel(f"[error]Database error pausing simulation for {tracking_number}: {e}[/error]", title="Database Error", border_style="red"))
...
def resume_simulation_callback(call, tracking_number, page):
    ...
    try:
        ...
        bot_logger.error(f"Database error resuming simulation: {e}", extra={'tracking_number': tracking_number})
        console.print(Panel(f"[error]Database error resuming simulation for {tracking_number}: {e}[/error]", title="Database Error", border_style="red"))
